...
class CorkamistyleSVGExporter:
    ...
    def _render_hex_dump(self, raw_data: bytes, fields: List[Dict], y_start: int) -> Tuple[int, Dict[int, int]]:
        ...
        # Layout positions - carefully calculated for alignment
        x_offset = self.margin_x
        x_hex = x_offset + 50  # After "0000:" offset column
        
        # Calculate hex section width: 16 bytes * width + gap between 7th and 8th
        hex_section_width = (self.hex_cols * self.hex_byte_w) + 15  # 15px gap in middle
        x_ascii = x_hex + hex_section_width + 15  # Gap before ASCII
        
        # Store the end of ASCII section for use by other components
        self.hex_section_end_x = x_ascii + (self.hex_cols * self.ascii_byte_w)
        ...
        # Column headers on next line
        header_y = y_start + 20
        
        # Build hex column header with proper spacing to match data alignment
        hex_header_parts = []
        for i in range(16):
            hx = x_hex + (i * self.hex_byte_w)
            if i >= 8:
                hx += 15  # Same gap as data
            hex_header_parts.append(f'<text x="{hx}" y="{header_y}" class="field-label">{i:02X}</text>')
        
        self.svg_parts.append(f'<text x="{x_offset}" y="{header_y}" class="field-label">OFFSET</text>')
        self.svg_parts.extend(hex_header_parts)
        self.svg_parts.append(f'<text x="{x_ascii}" y="{header_y}" class="field-label">ASCII</text>')
        
        # Divider line
        divider_y = header_y + 8
        self.svg_parts.append(f'<line x1="{x_offset}" y1="{divider_y}" x2="{self.hex_section_end_x}" y2="{divider_y}" class="divider"/>')
        ...
        for row_item in rows_to_render:
            if row_item == 'SKIP':
                ...
                skip_x = x_hex + (8 * self.hex_byte_w) + 7  # Center of hex area
                ...
                for col_idx, byte in enumerate(chunk):
                    ...
                    # Hex byte position - aligned with header
                    bx = x_hex + (col_idx * self.hex_byte_w)
                    if col_idx >= 8:
                        bx += 15  # Gap between 7th and 8th byte
    ...
    def _render_field_table(self, fields: List[Dict], raw_data: bytes, 
        ...
        for f_idx, field in enumerate(fields):
            ...
            if start is not None and byte_row_map and hasattr(self, 'hex_section_end_x'):
                ...
                if hex_y is not None:
                    ...
                    # Bracket positioned right after ASCII section
                    bracket_x = self.hex_section_end_x + 10
                    self.svg_parts.append(f'''
                    ...
                    # Bezier curve connector to field table
                    cx1 = bracket_x + 35
                    cx2 = connector_x - 35
    ...
    def export(self, flattened_data: List[Dict], raw_data: bytes, 
        ...
        """
        self.svg_parts = []
        self.hex_section_end_x = 0  # Will be set by _render_hex_dump
        ...
        hex_header_height = 50  # Section header + column headers + spacing
        hex_content_height = (hex_rows * self.hex_row_h) + (skip_rows * 14)
        hex_height = hex_header_height + hex_content_height
        
        # Field table height
        field_header_height = 50  # Same as hex for alignment
        ...
        # Hex dump (left side) - this sets self.hex_section_end_x
        hex_end_y, byte_map = self._render_hex_dump(raw_data, fields, self.margin_y)
        
        # Calculate table position: after hex section with connector space
        # Add space for bracket connectors (about 80px)
        table_x = self.hex_section_end_x + 100
